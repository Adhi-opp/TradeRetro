...
| **Idempotent Aggregation** | Silver re-aggregation upserts on the bar's primary key — `ON CONFLICT (instrument_key, bucket) DO UPDATE` — so re-processing the same 1-min bucket (rolling 5-min look-back) is safe and self-correcting. Bronze is an **append-only** tick log with no natural dedup key, so idempotency is enforced at the silver layer, not bronze (the consumer's `ON CONFLICT DO NOTHING` is a defensive guard that, absent a unique constraint, never fires). |
