...
| **Low temperature (0.2)** | Trading analysis needs precision, not creativity. A lower temperature reduces hallucinations and produces more deterministic outputs. Note: this value is delivered to provider payloads via `AIProviderFactory` (see [AI_CONFIGURATION.md](AI_CONFIGURATION.md)). |
